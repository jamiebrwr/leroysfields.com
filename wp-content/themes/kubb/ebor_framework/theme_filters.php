...
function _s_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'kubb' ), max( $paged, $page ) );
...
function ebor_post_gallery( $output, $attr) {
    ...
    foreach ( $attachments as $id => $attachment ) {
        ...
        $output .= '<figure><a href="'. $link .'" class="fancybox-media" data-rel="portfolio"><div class="text-overlay"><div class="info">'. get_option('blog_view_larger','View Larger') .'</div></div>' . wp_get_attachment_image($id, $size) . '</a></figure>';
