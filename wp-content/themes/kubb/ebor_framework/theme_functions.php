...
add_shortcode('kubb_sharing_buttons', 'kubb_sharing_buttons');
...
function ebor_page_title($title, $image = false){
	if( $image == false )
		$image = '';
		
	$output = "<div class='page-title' style='background-image: url(" . $image . ");'><div class='overlay'></div><h1>" . $title . "</h1></div>";
	return $output;	
}
...
function ebor_portfolio_post_select($field_id, $block_id, $options, $selected) {
	$output = '<select id="'. $block_id .'_'.$field_id.'" name="aq_blocks['.$block_id.']['.$field_id.']">';
	foreach($options as $option) {
...
function ebor_add_customize_page() {
	add_dashboard_page( 'Kubb Theme Options', 'Kubb Theme Options', 'edit_theme_options', 'customize.php' );
}
...
if(!( function_exists('tcb_add_post_thumbnail_column') )){
	function tcb_add_post_thumbnail_column($cols){
	  $cols['tcb_post_thumb'] = __('Featured Image','kubb');
	  return $cols;
	}
}
...
if(!( function_exists('ebor_pagination') )){
	function ebor_pagination($pages = '', $range = 2){
		...
		if(1 != $pages){
			...
			if($paged > 2 && $paged > $range+1 && $showitems < $pages) $output .= "<li><a href='".get_pagenum_link(1)."'>".__('First','kubb')."</a></li> ";
			...
			if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) $output .= "<li><a href='".get_pagenum_link($pages)."'>".__('Last','kubb')."</a></li> ";
			$output.= "</ul></div>";
		}
		    ...
		    <div class="bordered">
		      ...
		      <?php 
		      	...
		      	if ($comment->comment_approved == '0')
		      		echo '<p><em>'. __('Your comment is awaiting moderation.', 'kubb') .'</em></p>';
		      ?>
